            ...
            Object.values(calculatedDepData).map((item, index) => {
              return (
                <Trow key={`tr_${index}`}>
                  ...
                  <Tcell>
                    {item.error ? <StatusBadge>Unavailable</StatusBadge> : item.pkgNewVer ? <><span>{item.pkgNewVer}</span>{item.versionDetails && <StatusBadge $color={item.versionDetails.color}>{item.versionDetails.value}</StatusBadge>}</> : <Loader />}
                  </Tcell>
                </Trow>
              );
            })}
