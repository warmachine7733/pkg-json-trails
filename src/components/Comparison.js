...
function Comparison() {
  ...
  const applyFilters = (data) => Object.fromEntries(
    Object.entries(data || {})
      ...
      .sort(([, left], [, right]) => {
        const order = { Major: 0, Minor: 1, Patch: 2, Latest: 3, Unavailable: 4, Pending: 5 };
        const leftOrder = order[left.versionDetails?.value] ?? order.Pending;
        const rightOrder = order[right.versionDetails?.value] ?? order.Pending;
  ...
  const formatReportItems = (items) => items.length
    ? items.map((item) => {
      const status = item.error
        ? "UNAVAILABLE"
        : item.versionDetails?.value || "PENDING";
      return `- ${item.pkgName}\n  Current: ${item.pkgCurrVer}\n  Latest:  ${item.pkgNewVer || "Unavailable"}\n  Status:   ${status}`;
    }).join("\n\n")
    : "None";
  const formatGroupedReport = (items) => {
    const groups = ["Major", "Minor", "Patch", "Latest", "Unavailable", "Pending"];
    return groups.map((group) => {
      const groupItems = items.filter((item) => {
        const status = item.error ? "Unavailable" : item.versionDetails?.value || "Pending";
        return status === group;
      });
      return `${group.toUpperCase()} (${groupItems.length})\n${"-".repeat(group.length + 5)}\n${formatReportItems(groupItems)}`;
    }).join("\n\n");
  };
  const reportText = [
    ...
    "=======================",
    "",
    `Packages scanned: ${counts.All}`,
    `Updates available: ${updatedCount}`,
    `Major: ${counts.Major || 0} | Minor: ${counts.Minor || 0} | Patch: ${counts.Patch || 0} | Current: ${counts.Latest || 0}`,
    "",
    "ALL DEPENDENCIES",
    "----------------",
    formatGroupedReport(allItems),
    "",
    "Generated by Package Tracker",
  ].join("\n");
  const exportReport = () => {
